# 🇮🇳 Indian Family Law Drafting — Gemini CLI Port

> The same `indian-family-drafting` pipeline you know, now invokable from Gemini CLI. **Same 6-agent pipeline. Same canonical agent specs. Same filing-grade output. Different LLM brain.**
...
├── skills/ (personal-law-regime templates)                   ← case-type templates (unchanged)
├── .gemini/                         ← NEW
│   └── commands/
...
```
<case-folder>/
├── family-config.md           ← advocate-supplied (required)
